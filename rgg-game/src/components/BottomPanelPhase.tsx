...
const BottomPanelPhase: React.FC<BottomPanelPhaseProps> = ({
  ...
  const handleTestGetCards = async () => {
    ...
    try {
      ...
      console.error("Ошибка при доборе тестовых карт:", error);
    }
  };
  ...
  const handleGiveAllCards = async () => {
    ...
    try {
      ...
      const targetPlayers = players.filter((player) => player.role !== "admin");

      const updates = targetPlayers.map((player) =>
      ...
      alert("Ошибка при выдаче всех карт: " + message);
    }
  };

  const handleToggleResultsForm = () => {
    if (!isFillingResults) {
      setTempScores(gameState.currentResults || {});
    }
    setIsFillingResults((prev) => !prev);
  };
  ...
  return (
    <div
      className="w-full h-40 border-t border-purple-500/20 bg-black/40 backdrop-blur-md flex flex-col"
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-purple-500/10 gap-3">
        ...
        <div className="text-sm text-zinc-200 font-medium truncate">
          {gameState.phase === "waiting_game" &&
            `Этап: Ожидание проведения игры "${gameState.currentGame || "..."}" | Этап ${gameState.round}`}
          {gameState.phase === "playing" &&
            `Этап: Играем в "${gameState.currentGame || "..."}" | Этап ${gameState.round}`}
          {gameState.phase === "results" &&
            `Этап: Подводим итоги игры | Этап ${gameState.round}`}
          {gameState.phase === "voting" &&
            `Этап: Ожидаем пока игроки проголосуют... | Этап ${gameState.round}`}
        ...
        {gameState.currentRoll !== null && gameState.rollConfirmed && (
          <div className="flex items-center gap-2 bg-purple-900/50 px-3 py-1 rounded">
            <span className="text-sm text-purple-300">Ход: {gameState.currentRoll}</span>
          </div>
        )}
          ...
          <div className="flex gap-2 font-bold shrink-0">
            <button
              onClick={() => {
                if (
                  window.confirm(
                    "Загрузить стартовые карты в Firestore? Это обновит существующие данные.",
                  )
                ) {
                  uploadStarterCards()
            ...
            {gameState.phase === "next_game" && (
              <button
                onClick={() => {
                  console.log("Нажатие на колесо. Пропс onToggleWheel существует?", !!onToggleWheel);
                  onToggleWheel?.();
                }}
            ...
            {gameState.phase === "results" && (
              <div className="flex gap-2">
                <button
                  onClick={handleToggleResultsForm}
      ...
      <div className="flex-1 px-4 py-3 text-base text-zinc-200 flex items-center relative z-20 overflow-visible">
        {isAdmin && gameState.phase === "turn" && gameState.turnOrder.length > 0 && (
          <div className="flex flex-col gap-2 w-full animate-in fade-in slide-in-from-left-4 duration-500 overflow-hidden">
            <span className="text-[10px] font-black uppercase text-purple-400 tracking-widest px-1">
              Очередь ходов:
            </span>
                      ...
                      className={`
                        ...
                        ${
                          isCurrent
                            ...
                            : isDone
                              ? "bg-zinc-800/40 border-zinc-700 opacity-50"
          ...
          <div className="flex gap-4 items-center min-w-max animate-in fade-in slide-in-from-bottom-2 duration-300">
            {players
              ...
              .map((player) => (
                <div
                  key={player.id}
                  className="bg-zinc-900/60 p-2 rounded-xl border border-purple-500/20 flex flex-col items-center gap-1 min-w-[100px]"
                >
                  <span className="text-[10px] uppercase font-black text-zinc-500 truncate w-20 text-center">
                    {player.login}
                  </span>
                  <input
                    type="number"
                    ...
                    onChange={(event) =>
                      setTempScores((prev) => ({
                        ...prev,
                        [player.id]: parseInt(event.target.value, 10) || 0,
                      }))
                    }
                  />
                </div>
              ...
              {me?.inventory && me.inventory.length > 0 ? (
                ...
                >
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    ...
                    <span className="text-[7px] text-white/70 uppercase font-bold tracking-tighter">
                      В колоде
                    </span>
                  </div>
...
const PlayerVotingView: React.FC<{
  currentUser: User | null;
  players: Player[];
  gameState: GameState;
  ...
  const myPlayerData = players.find((player) => player.id === myId);
  ...
  if (!isParticipating) {
    return (
      <div className="text-zinc-500 italic">
        Вы не участвовали в этой игре, поэтому не можете голосовать.
      </div>
    );
  }

  if (hasVoted) {
    const votedFor = players.find((player) => player.id === gameState.votes?.[myId]);
    return (
  ...
  const candidates = players.filter(
    (player) => player.role !== "admin" && (player.lastTiltoCoins ?? 0) > 0,
  );
...
const AdminVotingView: React.FC<{
  gameState: GameState;
  players: Player[];
  onFinish: () => void;
}> = ({ gameState, players, onFinish }) => {
  const [showDetails, setShowDetails] = useState(false);

  const handleFinishVoting = async () => {
    const currentVotes = gameState.votes || {};

    const currentNonVoters = players.filter(
      (player) =>
        player.role !== "admin" && (player.lastTiltoCoins ?? 0) > 0 && !currentVotes[player.id],
    );

    let skipBonuses = false;
    if (currentNonVoters.length > 0) {
      const names = currentNonVoters.map((player) => player.login).join(", ");
      if (
        !window.confirm(
          `Точно завершить голосование? Еще не проголосовали: ${names}.\nВ этом случае бонусы начислены не будут.`,
        )
      ) {
        return;
      }
    ...
    if (!skipBonuses) {
      ...
      if (voteEntries.length > 0) {
        ...
        const winners = voteEntries.filter(([, count]) => count === maxVotes).map(([id]) => id);
  ...
  const nonVoters = players.filter(
    (player) =>
      player.role !== "admin" && (player.lastTiltoCoins ?? 0) > 0 && !gameState.votes?.[player.id],
  );
        ...
        <div className="fixed bottom-44 right-4 bg-zinc-900 border border-indigo-500/30 p-4 rounded-2xl shadow-2xl min-w-[200px] animate-in slide-in-from-right-4 duration-300">
          ...
          <div className="space-y-2">
            {players
              .filter((player) => player.role !== "admin")
              .map((player) => {
                const count = Object.values(gameState.votes || {}).filter((id) => id === player.id).length;
                if (count === 0) return null;
                return (
                  <div key={player.id} className="flex justify-between items-center text-xs">
                    <span className="text-zinc-300">{player.login}</span>
                    <span className="bg-indigo-500 text-white px-2 py-0.5 rounded-full font-bold">
                      {count}
                    </span>
                  </div>
                );
            ...
            <div className="mt-4 pt-3 border-t border-white/10">
              ...
              <div className="flex flex-wrap gap-1">
                {nonVoters.map((player) => (
                  <span
                    key={player.id}
                    className="text-[10px] text-zinc-400 bg-zinc-800 px-1.5 py-0.5 rounded border border-white/5"
                  >
                    {player.login}
                  </span>
                ))}
